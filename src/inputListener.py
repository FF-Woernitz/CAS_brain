        ...
        def _isActiveTimeNow(data, name):
            ...
            for d in data:
                ...
                if isinstance(d, bool):
                    ...
                    if len(d) == 0:
                        self.logger.debug("Trigger {}: Dict is empty. Ignore", name)
                        continue
                    for k, v in d.items():
                        if k == "weekday":
                            if v != now.weekday():
                            ...
                            self.logger.warning("Found unknown key {} with value {} in active of trigger", k, v)
                    return True
                else:
                    self.logger.warning("Found unknown type {} with value {} in active of trigger", name, type(d), d)
            return False

        self.logger.debug("Check if trigger {} is active", trigger["name"])
        inactive = False
        data = []
        if "active" not in trigger:
            if "inactive" not in trigger:
                self.logger.debug("Trigger {} has no active nor inactive key in config. So it es active",
                                  trigger["name"])
                return True
